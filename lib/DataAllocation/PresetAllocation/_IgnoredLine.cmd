...
::  !_cdc_Type!             -> [string]         : Define defined type's
...
::  !_cdc_isType_Unknown!   -> [boolean]        : Is the type defined?
::  !_cdc_isCodec_Unknown!  -> [boolean]        : Is the codec defined in type?
::  !_cdc_Video_Codec!      -> [string]         : What of current video codec type that you want to use?
::  !_cdc_Audio_Codec!      -> [string]         : What of current audio codec type that you want to use?
...
:RESET_TO_DEFAULT
    ...
    set _cdc_isSource=false
    set _cdc_isUnknown=false
    set _cdc_isCodec_Unknown=false
    set _cdc_Video_Codec=.
    set _cdc_Audio_Codec=.
...
REM                                 :: Display c_HASH To Output :3
                        ...
                        :IgnoredLine_c_AT
                            if "!_IgnoredLine_c_AT!" == "true" (
                                ...
                                :_cdc_Check_Types
                                    for /f "tokens=1,* delims=@ " %%a in ('echo !_inIl!') do (
                                        find "%%a" "lib\\Rulesets\\Preset\\_TypesOfCodec" > nul
                                        if !errorlevel! == 0 (
                                            REM Define %%a as defined type
                                            REM Define %%b as type of codec
                                            set _cdc_Type=%%a
                                            set _cdc_Codec=%%b
